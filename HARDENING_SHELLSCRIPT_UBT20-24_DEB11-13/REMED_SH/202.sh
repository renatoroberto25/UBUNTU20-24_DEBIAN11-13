...
need_root
awk -F: '$3>=1000 && $1!="nobody"{print $6}' /etc/passwd | while read -r h; do [ -d "$h" ] && find "$h" -maxdepth 1 -type f -name '.*' -perm /022 -exec chmod go-w {} + 2>/dev/null; done
echo "OK"
