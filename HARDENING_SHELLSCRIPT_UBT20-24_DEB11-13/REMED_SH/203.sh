...
need_root
awk -F: '$3>=1000 && $1!="nobody"{print $6}' /etc/passwd | while read -r h; do [ -d "$h" ] || continue; rm -f "$h/.forward" "$h/.rhosts"; [ -f "$h/.netrc" ] && chmod 600 "$h/.netrc"; done
echo "OK"
