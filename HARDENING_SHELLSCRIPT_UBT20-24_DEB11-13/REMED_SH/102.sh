...
need_root
conf=/etc/modprobe.d/hitss-hardening-bluetooth.conf
backup_file "$conf"
printf 'blacklist bluetooth\ninstall bluetooth /bin/true\n' > "$conf"
modprobe -r bluetooth >/dev/null 2>&1 || true
conf=/etc/modprobe.d/hitss-hardening-iwlwifi.conf
backup_file "$conf"
printf 'blacklist iwlwifi\ninstall iwlwifi /bin/true\n' > "$conf"
modprobe -r iwlwifi >/dev/null 2>&1 || true
echo "OK"
