...
ID=8
TITLE='Desabilitar overlayfs quando nao for requisito'
...
need_root
conf=/etc/modprobe.d/hitss-hardening-overlay.conf
backup_file "$conf"
printf 'blacklist overlay\ninstall overlay /bin/true\n' > "$conf"
modprobe -r overlay >/dev/null 2>&1 || true
echo "OK"
