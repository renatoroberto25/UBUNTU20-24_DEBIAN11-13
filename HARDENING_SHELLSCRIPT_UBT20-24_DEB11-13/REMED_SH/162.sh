...
backup_file "$file"
if grep -Eq '^[[:space:]]*#?[[:space:]]*Storage=' "$file"; then sed -i -E 's/^[[:space:]]*#?[[:space:]]*Storage=.*/Storage=persistent/' "$file"; else printf 'Storage=persistent\n' >> "$file"; fi
